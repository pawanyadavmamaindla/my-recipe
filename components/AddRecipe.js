...
const AddRecipe = ({ onExit , onAdd }) => {
    ...
    <div className="add-recipe">
        <h2>Add a Recipe</h2>
        <Input label="Title" onInput={(e) => dispatch({ type: 'update', payload: { title: 
            e.target.value } })} value={state.title} />
            <Input label="Description" onInput={(e) => dispatch({ type: 'update', payload: {
                    description: e.target.value } })} value={state.description} />
        <InputTags label="Keywords" onUpdate={(tags) => dispatch({ type: 'update', 
        payload: { keywords: tags } })} value={state.keywords} /> 
        <Select
        label="Type"
        ...
        />
        <List label="Ingredients" onUpdate={(items) => dispatch({ type: 'update', payload: 
        { ingredients: items } })} value={state.ingredients} />
        <Text
        label="Instructions"
